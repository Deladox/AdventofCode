...
from helper.ip_settings import IpSettingsSsh


class Ssh2PythonNb(IpSettingsSsh):
    def connect_session(self, timeout=10):
        ...
        while timeout_ >= time.time():
            try:
                ...
                sock.connect((self.ip_set, 22))
                session = Session()
                session.handshake(sock)
                session.userauth_password(username=self.user_set, password=self.pw_set)
            except:
                sock.close()
                time.sleep(5)
            else:
